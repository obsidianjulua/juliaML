...
=============================================================================
    ...
    function inject_debug(layer::MetaMorphicLayer)
        debug_fn = function(input, context)
            @info "Layer processing" input_size=size(get_hidden_state(input))
context_keys=keys(context)
            return input
        end
    ...
    function inject_visualizer(layer::MetaMorphicLayer; plot_every=10)
        ...
        viz_fn = function(input, context)
            ...
            if counter[] % plot_every == 0
                hidden = get_hidden_state(input)
                if length(hidden) <= 1000  # Reasonable size for plotting
                    display(heatmap(reshape(hidden, :, 1), title="Layer Output
Heatmap"))
                end
            end
            return input
        end
        inject_logic!(layer, viz_fn)
    end

    # Metamorphic model comparison
    function compare_models(original, modified, test_input)
        orig_out = original(test_input)
        mod_out = modified(test_input)

        return (
            original_output = orig_out,
            modified_output = mod_out,
            difference = norm(get_hidden_state(orig_out) -
get_hidden_state(mod_out)),
            similarity = dot(get_hidden_state(orig_out),
get_hidden_state(mod_out))
            )
    end

    # Performance tracking for metamorphic operations
    const METAMORPHIC_STATS = Dict{Symbol,Vector{Float64}}()

    function track_performance!(operation::Symbol, time::Float64)
        if !haskey(METAMORPHIC_STATS, operation)
            METAMORPHIC_STATS[operation] = Float64[]
        end
        push!(METAMORPHIC_STATS[operation], time)
    end

    macro timed_inject(expr)
    operation = Symbol(string(expr.args[1]))
    quote
        time_ns = @elapsed begin
            result = $(esc(expr))
        end
        track_performance!($(QuoteNode(operation)), time_ns)
        @info "Injection completed" operation=$($(QuoteNode(operation)))
time_ms=round(time_ns*1000, digits=2)
        result
    end
    end
...
=============================================================================
    ...
    begin
        ...
        HTML("""
             <div style="background: linear-gradient(45deg, #667eea 0%, #764ba2
100%);
             padding: 15px; border-radius: 10px; color: white;">
             <h3>🏗️ Model Architecture</h3>
             <p>Base Model: $(typeof(base_model))</p>
             <p>Layers: $(length(base_model.layers))</p>
             <p>MetaMorphic Layers: $(count(x -> x isa MetaMorphicLayer,
meta_model.layers))</p>
             </div>
             """)
    end
    ...
    begin
        ...
        @bind injection_text TextField(default="optimize attention and boost
performance")
    ...
    begin
        ...
        if inject_button > 0
            global modified_model, detected_patterns =
constructivist_training_session!(
                deepcopy(meta_model),
                injection_text,
            ...
            @info "Injection applied" patterns=detected_patterns
modifications=length(break_points)
        else
...
$(join(string.(detected_patterns), ", "))</p>
             <p><strong>Active Break Points:</strong>
$(join(string.(break_points), ", "))</p>
             <p><strong>Injection Status:</strong> $(inject_button > 0 ? "✅
Applied" : "⏳ Pending")</p>
             </div>
             """)
    end
    ...
    begin
        ...
        if @isdefined modified_model
            ...
            p1 = heatmap(original_output[1:min(20, size(original_output, 1)),
1:min(10, size(original_output, 2))],
                         title="Original Output", c=:viridis)

            p2 = heatmap(modified_output[1:min(20, size(modified_output, 1)),
1:min(10, size(modified_output, 2))],
                         title="MetaMorphic Output", c=:plasma)

            # Difference analysis
            diff_matrix = abs.(original_output - modified_output)
            p3 = heatmap(diff_matrix[1:min(20, size(diff_matrix, 1)), 1:min(10,
size(diff_matrix, 2))],
    ...
    begin
        ...
        if @isdefined modified_model
            ...
            performance_data = [
                ("Inference Time (ms)", original_time * 1000, modified_time *
1000),
            ...
            HTML("""
                 <div style="background: #1a202c; color: #e2e8f0; padding: 20px;
...
right;">$(round(orig, digits=3))</td>
                         <td style="padding: 8px; text-align:
right;">$(round(mod, digits=3))</td>
                         <td style="padding: 8px; text-align: right; color:
$(mod/orig > 1.1 ? "#fc8181" : mod/orig < 0.9 ? "#68d391" : "#a0aec0");">
                 ...
                 """)
                             ...
                             HTML("<p>Run injection to see performance
metrics...</p>")
                         end
        end
        ...
        begin
            ...
            if inject_button > 0 && @isdefined modified_model
                experiment_entry = (
                    ...
                    performance_ratio = @isdefined(modified_time) ?
modified_time / original_time : 1.0
                    )

                # Only add if not duplicate
                if isempty(experiment_history) ||
experiment_history[end].timestamp != experiment_entry.timestamp
            ...
            if !isempty(experiment_history)
                HTML("""
                     <div style="background: #2d3748; color: #e2e8f0; padding:
15px; border-radius: 10px; max-height: 300px; overflow-y: auto;">
                     <h4>🔬 Experiment History</h4>
                     $(join(["""
                             <div style="border: 1px solid #4a5568; margin: 5px
0; padding: 10px; border-radius: 5px;">
                             <strong>$(exp.timestamp)</strong><br>
                             <em>Input:</em> "$(exp.injection_text)"<br>
                             <em>Break Points:</em>
$(join(string.(exp.break_points), ", "))<br>
                             <em>Patterns:</em> $(join(string.(exp.patterns), ",
"))<br>
                             <em>Performance:</em>
$(round(exp.performance_ratio, digits=3))x
                             </div>
                             """ for exp in reverse(experiment_history[max(1,
end-4):end])]))
                     </div>
                     """)
                             else
                                 HTML("<p><em>No experiments logged yet. Apply
an injection to start logging.</em></p>")
                             end
            end
                    ...
                    if @isdefined modified_model
                        ...
                        for layer in modified_model.layers
                            if layer isa MetaMorphicLayer
                                metamorphic_count += 1
                                total_modifications +=
length(layer.active_modifications)
                                println("Layer $(metamorphic_count):
$(length(layer.active_modifications)) modifications")
                            end
                        end
                        ...
                        println("  Injection Efficiency:
$(round(total_modifications/max(1,metamorphic_count), digits=2))
modifications/layer")

                        if haskey(METAMORPHIC_STATS, :injection)
                            avg_injection_time =
mean(METAMORPHIC_STATS[:injection]) * 1000
                            println("  Avg Injection Time:
$(round(avg_injection_time, digits=2)) ms")
                        end
                    else
